...
// ========== /api/usage：返回 GLM + 百度用量 ==========
async function handleUsage(env, corsHeaders) {
  const glm = await getGLMUsage(env);
  const baidu = await getBaiduUsage(env);
  return json({ glm, baidu }, corsHeaders);
}
...
const GLM_QUOTA = 2_000_000;
const BAIDU_QUOTA = 50_000;

async function getGLMUsage(env) {
  const key = getMonthKey();
  const used = Number(await env.GLM_USAGE?.get(key) || 0);
  return {
    used,
    quota: GLM_QUOTA,
    percent: Math.min(100, Math.round((used / GLM_QUOTA) * 10000) / 100),
  };
}

async function getBaiduUsage(env) {
  const key = 'baidu:' + getMonthKey();
  const used = Number(await env.BAIDU_USAGE?.get(key) || 0);
  return {
    used,
    quota: BAIDU_QUOTA,
    percent: Math.min(100, Math.round((used / BAIDU_QUOTA) * 10000) / 100),
  };
}
